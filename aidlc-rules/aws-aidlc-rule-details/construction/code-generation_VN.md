# Tạo Mã (Code Generation) - Các bước Chi tiết

## Tổng quan

Giai đoạn này tạo mã cho mỗi đơn vị công việc thông qua hai phần tích hợp:
...
- Tạo Thiết kế Đơn vị phải hoàn tất cho đơn vị
- Triển khai NFR (nếu đã thực thi) phải hoàn tất cho đơn vị
- Tất cả các artifact thiết kế đơn vị phải có sẵn
- Đơn vị đã sẵn sàng để tạo mã
...
- [ ] Đọc artifact thiết kế đơn vị từ Tạo Thiết kế Đơn vị
- [ ] Đọc bản đồ story đơn vị để hiểu các stories được giao
- [ ] Xác định các phụ thuộc và giao diện của đơn vị
- [ ] Xác thực đơn vị đã sẵn sàng để tạo mã
...
- [ ] **Chỉ Brownfield**: Xem xét reverse engineering code-structure.md cho các tệp hiện có để sửa đổi
- [ ] Ghi lại đường dẫn chính xác (không bao giờ là aidlc-docs/)
- [ ] Tạo các bước rõ ràng cho việc tạo đơn vị:
  - Thiết lập Cấu trúc Dự án (chỉ greenfield)
  - Tạo Logic Nghiệp vụ
  - Unit Testing cho Logic Nghiệp vụ
  - Tóm tắt Logic Nghiệp vụ
  - Tạo Lớp API
  - Unit Testing cho Lớp API
  - Tóm tắt Lớp API
  - Tạo Lớp Repository
  - Unit Testing cho Lớp Repository
  - Tóm tắt Lớp Repository
  - Tạo Frontend Components (nếu áp dụng)
  - Unit Testing cho Frontend Components (nếu áp dụng)
  - Tóm tắt Frontend Components (nếu áp dụng)
  - Script Di chuyển Cơ sở dữ liệu (nếu mô hình dữ liệu tồn tại)
  - Tạo Tài liệu (API docs, cập nhật README)
  - Tạo Artifact Triển khai
- [ ] Đánh số mỗi bước theo trình tự
- [ ] Bao gồm các tham chiếu ánh xạ story
- [ ] Thêm các hộp kiểm [ ] cho mỗi bước

## Bước 3: Bao gồm Ngữ cảnh Tạo Đơn vị

- [ ] Đối với đơn vị này, bao gồm:
  - Stories được triển khai bởi đơn vị này
  - Phụ thuộc vào các đơn vị/dịch vụ khác
...
- [ ] Nhấn mạnh rằng kế hoạch này là nguồn sự thật duy nhất cho Tạo Mã

## Bước 5: Tóm tắt Kế hoạch Đơn vị

- [ ] Cung cấp tóm tắt kế hoạch tạo mã đơn vị cho người dùng
- [ ] Làm nổi bật cách tiếp cận tạo đơn vị
- [ ] Giải thích trình tự bước và bao phủ story
- [ ] Ghi chú tổng số bước và phạm vi ước tính

## Bước 6: Ghi nhật ký Lời nhắc Phê duyệt
...
- [ ] Không tiếp tục cho đến khi người dùng phê duyệt kế hoạch tạo mã đơn vị một cách rõ ràng
...
- [ ] Đánh dấu Lập kế hoạch Mã hoàn tất trong `aidlc-state.md`
...
- [ ] Xác định bước chưa hoàn thành tiếp theo (hộp kiểm [ ] đầu tiên)
- [ ] Tải ngữ cảnh cho bước đó (đơn vị, phụ thuộc, stories)

## Bước 11: Thực thi Bước Hiện tại

- [ ] Xác minh thư mục đích từ kế hoạch (không bao giờ là aidlc-docs/)
- [ ] **Chỉ Brownfield**: Kiểm tra nếu tệp đích tồn tại
...
- [ ] Đánh dấu các story đơn vị liên quan là [x] khi việc tạo chúng hoàn tất
- [ ] Cập nhật trạng thái hiện tại trong `aidlc-docs/aidlc-state.md`
- [ ] **Chỉ Brownfield**: Xác minh không có tệp trùng lặp được tạo (ví dụ: không có `ClassName_modified.java` bên cạnh `ClassName.java`)
- [ ] Lưu tất cả các artifact đã tạo

## Bước 13: Tiếp tục hoặc Hoàn thành Tạo Mã

- [ ] Nếu còn bước, quay lại Bước 10
- [ ] Nếu tất cả các bước hoàn tất, tiến hành trình bày thông điệp hoàn thành
...
```
     ...
     2. **Tóm tắt AI** (tùy chọn): Cung cấp tóm tắt gạch đầu dòng có cấu trúc
        - **Brownfield**: Phân biệt tệp sửa đổi vs tệp tạo mới (ví dụ: "• Modified: `src/services/user-service.ts`", "• Created: `src/services/auth-service.ts`")
        - **Greenfield**: Liệt kê các tệp đã tạo với đường dẫn (ví dụ: "• Created: `src/services/user-service.ts`")
        - Liệt kê test, tài liệu, artifact triển khai với đường dẫn
        - Giữ tính thực tế, không hướng dẫn quy trình làm việc
...
- Không tiếp tục cho đến khi người dùng phê duyệt mã đã tạo một cách rõ ràng
...
- Đánh dấu giai đoạn Tạo Mã là hoàn tất cho đơn vị này trong aidlc-state.md
...
- **Mã ứng dụng**: Chỉ thư mục gốc workspace (KHÔNG BAO GIỜ là aidlc-docs/)
...
- **Greenfield đơn vị đơn**: `src/`, `tests/`, `config/` trong thư mục gốc workspace
- **Greenfield đa đơn vị (microservices)**: `{unit-name}/src/`, `{unit-name}/tests/`
- **Greenfield đa đơn vị (monolith)**: `src/{unit-name}/`, `tests/{unit-name}/`
...
- Ghi lại ngữ cảnh và phụ thuộc của đơn vị
- Nhận phê duyệt rõ ràng của người dùng trước khi tạo

### Quy tắc Giai đoạn Tạo

- **KHÔNG CÓ LOGIC CỨNG NHẮC**: Chỉ thực thi những gì được viết trong kế hoạch đơn vị
- **TUÂN THEO KẾ HOẠCH CHÍNH XÁC**: Không đi chệch khỏi trình tự bước
- **CẬP NHẬT HỘP KIỂM**: Đánh dấu [x] ngay lập tức sau khi hoàn thành mỗi bước
- **TRUY VẾT STORY**: Đánh dấu các story đơn vị [x] khi chức năng được thực hiện
- **TÔN TRỌNG PHỤ THUỘC**: Chỉ thực hiện khi các phụ thuộc đơn vị được thỏa mãn

### Quy tắc Mã Thân thiện Tự động hóa

Khi tạo mã UI (web, mobile, desktop), đảm bảo các phần tử thân thiện với tự động hóa:

- Thêm thuộc tính `data-testid` vào các phần tử tương tác (buttons, inputs, links, forms)
- Sử dụng cách đặt tên nhất quán: `{component}-{element-role}` (ví dụ: `login-form-submit-button`, `user-list-search-input`)
- Tránh các ID động hoặc tự động tạo thay đổi giữa các lần render
...
- Tất cả các story đơn vị được thực hiện theo kế hoạch
- Tất cả mã và test được tạo (test sẽ được thực thi trong giai đoạn Xây dựng & Kiểm thử)
- Artifact triển khai được tạo
- Đơn vị hoàn chỉnh sẵn sàng để xây dựng và xác minh
